...
export default function AutoMailSettingsScreen({ onLogout }) {
  ...
  const [groups, setGroups] = useState({ arrears:[], overdue:[], upcoming:[], advance:[] });
  ...
  const load = useCallback(async () => {
    try {
      ...
      setGroups(next);
    } catch (error) { Alert.alert('Auto mail error', getMessage(error)); }
  ...
  const save = async () => {
    if (!validate()) return;
    setSaving(true);
    try { await autoMailApi.saveConfig(config); Alert.alert('Saved', 'Auto email settings updated successfully.'); }
    catch (error) { Alert.alert('Save failed', getMessage(error)); }
    finally { setSaving(false); }
  };

  if (loading) return <ScreenSkeleton header cards={3} cardHeight={150} />;
  ...
  return <View style={styles.screen}>
    ...
    <ScrollView contentContainerStyle={styles.body} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => {setRefreshing(true);load();}} />}>
      <AppCard><View style={styles.masterRow}><View style={styles.masterIcon}><Icon name="lightning-bolt-outline" size={24} color={colors.violet} /></View><View style={styles.flex}><Text style={styles.cardTitle}>Master Scheduler</Text><Text style={styles.muted}>Turn the entire automation system on or off</Text></View><Switch value={config.isEnabled} onValueChange={value => set('isEnabled',value)} trackColor={{false:'#d1d5db',true:'#a78bfa'}} thumbColor={config.isEnabled?colors.violet:'#f3f4f6'} /></View></AppCard>
      <AppCard><View style={styles.cardHead}><Icon name="bell-outline" size={23} color={colors.accent}/><Text style={styles.cardTitle}>Reminder Policies</Text></View>
      ...
      <AppCard><View style={styles.cardHead}><Icon name="email-check-outline" size={23} color={colors.success}/><View><Text style={styles.cardTitle}>Targeted Mail Status</Text><Text style={styles.muted}>Groups configured to receive reminders</Text></View></View>
        {noPolicies ? <View style={styles.noPolicies}><Icon name="email-off-outline" size={34} color={colors.muted}/><Text style={styles.noTitle}>No policies are active</Text></View> : <>
      ...
      </AppCard><View style={styles.footer}/>
    </ScrollView>
  </View>;
}
